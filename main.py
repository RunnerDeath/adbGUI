...
apk_file = []
layout = [
    [sg.Text('Выбранные Android-устройства'),
     sg.InputText(key='_ACTUAL_DEVICE_',readonly=True),
     sg.Button('Очистить',key='_CLEAR_')],

    [sg.Listbox(key='_DEV_LST_' ,values=devices_list(),size=(0,5), enable_events=True, select_mode=sg.LISTBOX_SELECT_MODE_SINGLE),
     sg.Button('Демонстрация экрана', key='_SHOW_SCREEN_'),
     sg.Listbox(key='_APP_LST_' ,values=[],size=(20,5), enable_events=True, select_mode=sg.LISTBOX_SELECT_MODE_SINGLE)],
    ...
    if event == '_INSTALL_':
        if len(device)>0 and len(apk_file)>0:
            ...
            for i in range(0, len(r)):
                sg.popup(f'Ошибка {r[i]}', title='Окошко')
        else:
            install_error_popup()
